...
export default function CreateEvent() {
  ...
  const handleSave = async () => {
    ...
    const newEvent: Event = {
      ...
      name: eventName,
      startTime: isAllDay
        ? new Date(eventStartTime.setHours(0, 0, 0, 0))
        : eventStartTime,
      endTime: isAllDay
        ? new Date(eventEndTime.setHours(23, 59, 59, 999))
        : eventEndTime,
          ...
          },
          {
            ...
            onPress: async () => {
              ...
              try {
                ...
                const utcStartTime = selectedEvent.startTime;
                const apiUID = `${formatUIDDate(utcStartTime)}-${
                  selectedEvent.name
                }`;
                ...
                const updatedEvents = events.filter(
                  (event) => event.uid !== selectedEvent.uid
                );
    ...
    <SafeAreaView>
      ...
      <View style={{ backgroundColor: "white", height: 120 }}>
        <Image
          ...
          style={{ left: 20, height: 120, width: 220 }}
        ></Image>
      </View>
        ...
        >
          <View style={styles.modalContainer}>
            <View style={styles.modalView}>
              <ThemedText
                type="title"
                style={[styles.modalText, styles.headingText]}
              >
                Create a New Event
              </ThemedText>
                  ...
                  <ThemedText style={styles.modalText}>
                    {selectedEvent.isAllDay
                      ...
                      : `${format(
                          selectedEvent.startTime,
                          "EEEE, MMMM d, yyyy"
                        )} ${formatTime(selectedEvent.startTime)} - ${format(
                          selectedEvent.endTime,
                          "EEEE, MMMM d, yyyy"
                        )} ${formatTime(selectedEvent.endTime)}`}
                  </ThemedText>
